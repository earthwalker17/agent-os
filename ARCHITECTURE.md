...
| `git_ops.py` / `github_connector.py` / `_git_askpass.sh` | Sandboxed local Git via `run_git` (`ensure_repo`, tagged out-of-branch checkpoints, redacted bounded `capture_diff`, secret-refusing `commit`, `create_branch`, gated `rollback`; never raises into the run loop) + GitHub via REST/`urllib` (no `gh` CLI): `status`, tokenless `ensure_remote`, `push_branch`, `create_pull_request` — token injected ONLY via the generated `GIT_ASKPASS` env, never argv/`.git/config`. Output redacted. |
...
| `components/ProjectList.tsx` / `ContextPanel.tsx` / `RunsSection.tsx` | Left column (Agents button, projects + conversations + CRUD); right column (editable project memory files, env registry, runs list — auto-polls while active, live files/cmds counts, Start/Stop preview, per-row Trace). |
...
| `components/ChatPanel.tsx` (+ `ModelPicker.tsx`) | Center: header (provider + theme selectors), message thread, multi-modal composer (auto-grow textarea, file upload with "add to workspace", voice, `@`-autocomplete). Renders `RunChatCard` on `run_id` messages, research-sources + suggestion chips; the per-provider model picker gates image upload by the model's `vision` flag. |
...
| `components/EditModal / ConfirmDialog / GlobalMemoryModal / ConnectorsModal / ConnectorModal / EnvRegistryPanel` | Memory editing, confirmations, global-memory viewer; connector setup (write-only, presence-only display, Stripe TEST badge) + app-env registry with per-key "Push to Vercel". |
...
- **`SOUL.md`** is read-only + hidden — never shown, auto-written, or in any write path.
