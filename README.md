...
| **Browser verification + visual review** | Spins up the dev server, **waits for a genuine render** (not a loading spinner), captures the entry page plus declared views and a few discovered ones via headless Playwright — and can execute **declared interaction flows** (click / fill / submit / expect_text; bounded, same-origin, per-step screenshots, console/network evidence) — then has a **vision model review** the screenshots (`passed` / `warning` / `failed` / `inconclusive`). A failed declared flow fails the verification; the visual verdict stays diagnostic and skips gracefully without a vision-capable model. |
...
| **Recovery / retry / cancel** | Runs can be **cancelled** cooperatively and **retried** as a fresh linked run. A non-green run gets a best-effort, **typed recovery assessment** (Recovery Matrix: build / runtime / visual / …) whose follow-up task card carries the concrete failure **evidence** (screenshots, console errors, failed flow steps). With explicit user approval, a bounded **auto-recovery budget** (≤ 2, contract-clamped per failure type) lets a run self-heal — even a *visually* broken but technically green build — capped, linked, and fully audited. |
